...
class Q_Network(object):
    ...
    def _generate_Q_network(self, ith=0):
        ...
        self.main_placeholders[ith], self.main_Qs[ith] \
            = self._extend_params_to_network(self.main_params[ith], element_sizes)

        self.target_placeholders[ith], self.target_Qs[ith] \
            = self._extend_params_to_network(self.target_params[ith], element_sizes)
            ...
            for element in range(element_types):
                ...
                for element_g in range(element_types):
                    if element_sizes[element_g]:
                        ## previous version
                        if  element_g == 2 and FLAGS.AGENT == "dqn_individual":
                            pass
                        ...
                        # temp_g += tf.tensordot(temp_reduced, Wg[layer][element_g][element], axes=[[-1], [0]])c

                # if not FLAGS.AGENT=="dqn_individual":
                if temp_g !=0:
    ...
    def _generate_q_values(self):
        ...
        for ith in range(self.N_CHOOSE):
            ...
            self.main_q_values[ith] = tf.reduce_sum(tf.multiply(self.main_Qs[ith], tf.reshape(self.placeholders[ith][self.ACT_SEL_BUF],[-1, self.N_EQRT - ith, 1])), axis=1)
            ...
            if FLAGS.AGENT == "dqn" or FLAGS.AGENT =="dqn_myopic" or FLAGS.AGENT =="dqn_individual":
                self.target_q_values[ith] = tf.reshape(self.placeholders[ith][self.REWARD_BUF],[-1, ]) + 1 * tf.reduce_max(
                        self.target_Qs[(ith + 1) % self.N_CHOOSE], axis=[-2, -1])
                ## TODO centralized round robin
                ...
                self.target_q_values[ith] = tf.reshape(self.placeholders[ith][self.REWARD_BUF],[-1, ]) + 1 * tf.reduce_mean(tf.reduce_max(self.target_Qs[(ith + 1) % self.N_CHOOSE], axis=[-1]), axis = [1])
                # self.target_q_values[ith] = tf.reshape(self.placeholders[ith][self.REWARD_BUF],[-1, ]) + 1 * tf.reduce_max(self.target_Qs[(ith + 1) % self.N_CHOOSE], axis=[-1])[:,0] # 1/28 previous code

                if ith + 1 == self.N_CHOOSE:  # for last transitions
                    self.target_q_values[ith] = tf.reshape(self.placeholders[ith][self.REWARD_BUF],[-1, ]) + self.GAMMA * tf.reduce_mean(tf.reduce_max(self.target_Qs[(ith + 1) % self.N_CHOOSE], axis=[-1]), axis = [1])

                    # self.target_q_values[ith] = tf.reshape(self.placeholders[ith][self.REWARD_BUF],[-1, ]) + 1 * tf.reduce_max(self.target_Qs[(ith + 1) % self.N_CHOOSE], axis=[-1])[:,0] #1/28 previous
    ...
    def _sample_act_ith(self, obs_ith, eps=0, ith=0):
        ...
        if np.random.rand(1) > eps:
            ...
            if FLAGS.AGENT =="dqn" or FLAGS.AGENT =="dqn_myopic" or FLAGS.AGENT =="dqn_individual":
                ...
                ## TODO: centralized act_sel: random act_sub: N_SUBACT
                # two types of actions
                q_values_ith = self.sess.run(self.main_Qs[ith], feed_dict_act)
                # print('q_values_ith',q_values_ith, 'ith', ith)
                act_sel_ith = np.random.randint(0, self.N_EQRT - ith)
                # print('q_values_ith[0][act_sel_ith]',q_values_ith[0][act_sel_ith])
                act_sub_ith = np.argmax(q_values_ith[0][act_sel_ith])
                # act_sub_ith = pass
                pass
