...
def train(save_path):
    ...
    """
    print("N_EQUIVARIANT is {}".format(FLAGS.N_INPUT))
    ...
    fcsv, csv = open_csv(save_path)

    fcsv_rad, csv_rad = open_csv(save_path)
     ...
     (FLAGS.RELOAD_EP+1)*FLAGS.TRAIN_EPISODE):
        ...
        for step in range(FLAGS.TRAIN_STEP):
            ...
            if FLAGS.ENVIRONMENT == "circle_env_good":
                reward, radiuses = cp.deepcopy(env.step(action))
                for radius in radiuses:
                    radius_summary[int(radius * 100)] += 1
            else:
            ...
            if (FLAGS.TRAIN_STEP * episode + step+1) % FLAGS.SAVE_PERIOD == 0:
                ...
                # TODO csv write with reward here just sum every thing and averaged
                #  self._wrt_csv_row(self.wr_avg, csv_data)
                # NOTE what is avg_reward?
                # avg_reward = avg_reward / float(FLAGS.SAVE_PERIOD)
                # csv.writerow(avg_reward)
                csv_rad.writerow(radius_summary)
                radius_summary = np.zeros(46)
...
if __name__ == '__main__':
    ...
    # TODO FLAGS.INPUT only (50, 200) * (50, 200) 
    # TODO set the hyper-parameters: reload number, seeds, number of items to reload (50, 200),   
    # TODO just turn on test_mode with 10 repeat time
    # TODO Train several with different FLAGS.parameters --> change RELOAD 
    # TODO 

    Repeat_Time = 10

    AGENT_LIST = ['dqn-DIFFEINET','dqn-SHAREEINET','dqn-PROGRESSIVE','dqn_ind-SHAREEINET','dqn_myopic-DIFFEINET','dqn_individual-DIFFEINET']
    AGENT_LABELs = ['ISQ-I', 'ISQ-U', "ISQ-P",  'IDQN', 'MQ', 'EQ']
    # TODO path = define

    for seed in range(4):
        FLAGS.SEED = seed
        
        for algo_name in AGENT_LIST:
            path = FLAGS.ENVIRONMENT+'/radius_result/'+str(algo_name)+ '.csv'
            FLAGS.N_INPUT = 200
            FLAGS.N_EQUIVARIANT = 200
            for reload_num in range(Repeat_Time):
                FLAGS.RELOAD_EP = FLAGS.TOTAL_RELOAD - (Repeat_Time-1) + reload_num
                train(path)
