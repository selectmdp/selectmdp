...
class Environment(object):
    ...
    def step(self, action):
        ...
        self.update_before_select() 
        # update the circles with selected action, get reward
        self.eq_collision_list, self.in_collision_list =  self.update_with_action(action)   
        # generate new circle circles
        self.update_select_eq_elements(action)
    ...
    def update_eq_expand(self):
        ...
        """
        EXPAND_eq = 0.4* FLAGS.MAX_RADIUS * FLAGS.N_CHOOSE/np.float(self.N_EQ) # NOTE HS: just guess for expension, I want slow expension rate so the heurstic would destroy all middle size circle and going inefficient with training step 1000 
